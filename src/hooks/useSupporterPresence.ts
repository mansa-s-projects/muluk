...
interface useSupporterPresenceOptions {
  /** The supporter's SUPPORTER-XXXXXXXXXX code */
...
interface useSupporterPresenceReturn {
  /** Fire an activity event. Fire-and-forget — never throws. */
  logActivity: (type: ActivityType, opts?: LogActivityOptions) => void;
}

/**
 * Supporter-side hook that maintains a presence heartbeat and exposes logActivity().
 * Drop into any supporter-facing page component that has the supporter code available.
 *
 * @example
 * const { logActivity } = useSupporterPresence({ code: SupporterCode, currentPage: pathname });
 * // On vault open:
 * logActivity("vault_view", { contentId: item.id });
 */
export function useSupporterPresence({
  code,
  currentPage,
  sessionId,
}: useSupporterPresenceOptions): useSupporterPresenceReturn {
