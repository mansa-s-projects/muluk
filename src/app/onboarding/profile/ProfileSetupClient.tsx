...
type Props = {
  ...
  /** Called after a successful save. Parent decides what to do next. */
  onSaved?: () => void;
};
  ...
  const card = (
    <div style={{
      ...
      <div style={{
        ...
        {!bannerSrc && <span style={{ color: "rgba(200,169,110,0.2)", fontSize: showModal ? "32px" : "22px" }}>✦</span>}
      </div>

      <div style={{ padding: "0 20px 20px" }}>
        <div style={{ marginTop: "-28px", marginBottom: "12px" }}>
          <div style={{
            width: showModal ? "64px" : "52px", height: showModal ? "64px" : "52px",
            borderRadius: "50%", border: "3px solid #0f0f1e", background: "#1a1a30",
        ...
        {draft.handle && (
          <div style={{ ...mono, fontSize: "11px", color: "rgba(255,255,255,0.35)", marginBottom: "6px" }}>@{draft.handle}</div>
        )}
        {draft.specialty && (
          <div style={{
            display: "inline-flex", background: "rgba(200,169,110,0.08)",
            border: "1px solid rgba(200,169,110,0.18)", borderRadius: "3px",
            padding: "2px 8px", ...mono, fontSize: "9px", letterSpacing: "0.15em",
            color: "var(--gold-dim)", textTransform: "uppercase", marginBottom: "8px",
          }}>
            {draft.specialty}
          </div>
        )}
        {draft.bio && (
          <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.6)", lineHeight: 1.55, marginBottom: "12px", fontFamily: "var(--font-body)" }}>
            {draft.bio}
          </div>
        )}
        {draft.location && (
          <div style={{ ...mono, fontSize: "10px", color: "rgba(255,255,255,0.3)", marginBottom: "10px" }}>📍 {draft.location}</div>
        )}
        {(draft.ctaLabel || draft.ctaUrl) && (
          <div style={{
            marginTop: "12px", padding: "10px 16px", background: "var(--gold)",
            borderRadius: "4px", textAlign: "center", ...mono,
            fontSize: "11px", letterSpacing: "0.14em", color: "#0a0800", fontWeight: 500,
          }}>
            {draft.ctaLabel || "Visit Website"}
          </div>
        )}
      </div>
    </div>
  ...
  if (showModal) {
    return (
      <div onClick={onClose} style={{
        position: "fixed", inset: 0, zIndex: 100, background: "rgba(2,2,3,0.85)",
        ...
        <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: "440px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            ...
            <button onClick={onClose} style={{ background: "transparent", border: "none", color: "rgba(255,255,255,0.45)", cursor: "pointer", fontSize: "18px" }}>×</button>
          </div>
          {card}
        </div>
      </div>
...
// ── Main component ────────────────────────────────────────────────────────────
// Post-onboarding profile editor. Does NOT manage any onboarding flow.
// Onboarding profile setup is handled exclusively by OnboardingWizard step 4.
export default function ProfileSetupClient({ initialDraft, onSaved }: Props) {
  ...
  return (
    <div style={{ color: "rgba(255,255,255,0.92)" }}>
      ...
      <div style={{
        ...
        <div style={{ ...disp, fontSize: "20px", color: "var(--gold)", fontWeight: 300 }}>Edit Profile</div>
        <button
          onClick={() => setShowPreview(true)}
          style={{
            background: "transparent", border: "1px solid rgba(200,169,110,0.25)",
            borderRadius: "4px", padding: "7px 16px",
            ...mono, fontSize: "10px", letterSpacing: "0.14em", color: "var(--gold)", cursor: "pointer",
          }}
        >PREVIEW</button>
      </div>

      <div style={{
        maxWidth: "1100px", margin: "0 auto", padding: "40px 24px 100px",
        ...
        <div>
          ...
          <div style={{ marginBottom: "8px" }}>
            <div style={{
              ...
              background: (bannerPreview || draft.bannerUrl) ? "transparent" : "linear-gradient(135deg, rgba(200,169,110,0.05), rgba(200,169,110,0.01))",
              border: "1px solid rgba(255,255,255,0.08)", overflow: "hidden", position: "relative",
              display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              {(bannerPreview || draft.bannerUrl) ? (
                <img src={bannerPreview || draft.bannerUrl!} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              ) : (
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: "22px", opacity: 0.2, marginBottom: "6px" }}>✦</div>
                  <div style={{ ...mono, fontSize: "10px", color: "rgba(255,255,255,0.2)", letterSpacing: "0.14em" }}>BANNER IMAGE · OPTIONAL</div>
                </div>
              )}
              <div style={{ position: "absolute", bottom: "10px", right: "10px", display: "flex", gap: "8px" }}>
                <button onClick={() => bannerRef.current?.click()} style={{
                  ...
                  ...mono, fontSize: "10px", letterSpacing: "0.12em", color: "rgba(255,255,255,0.8)", cursor: "pointer",
                }}>
                  {(bannerPreview || draft.bannerUrl) ? "CHANGE" : "ADD BANNER"}
                </button>
                {(bannerPreview || draft.bannerUrl) && (
                  <button onClick={() => { setBannerFile(null); setBannerPreview(null); update("bannerUrl", null); }} style={{
                    padding: "6px 12px", background: "rgba(224,85,85,0.1)",
                    border: "1px solid rgba(224,85,85,0.25)", borderRadius: "4px",
                    ...mono, fontSize: "10px", letterSpacing: "0.12em", color: "#e05555", cursor: "pointer",
                  }}>REMOVE</button>
                )}
              </div>
            </div>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: "14px", marginBottom: "28px", marginTop: "-24px", paddingLeft: "16px" }}>
            <div style={{ position: "relative", flexShrink: 0 }}>
              <div style={{
                ...
                border: "3px solid var(--void)", background: "#1a1a30",
                overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center",
              }}>
            ...
            {(avatarPreview || draft.avatarUrl) && (
              <button onClick={() => { setAvatarFile(null); setAvatarPreview(null); update("avatarUrl", null); }} style={{
                padding: "4px 10px", background: "transparent",
                border: "1px solid rgba(224,85,85,0.28)", borderRadius: "4px",
                ...mono, fontSize: "10px", letterSpacing: "0.12em", color: "#e05555", cursor: "pointer", marginBottom: "4px",
              }}>REMOVE PHOTO</button>
            )}
          ...
          <div style={{ display: "grid", gap: "18px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              <Field label="Display Name *">
                <input type="text" value={draft.displayName} onChange={e => update("displayName", e.target.value)} placeholder="Your public name" style={inp} />
              </Field>
              <Field label="Handle">
                <div style={{ position: "relative" }}>
                  <span style={{ position: "absolute", left: "14px", top: "50%", transform: "translateY(-50%)", color: "rgba(255,255,255,0.25)", ...mono, fontSize: "14px" }}>@</span>
                  <input type="text" value={draft.handle}
                    onChange={e => update("handle", e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ""))}
                    placeholder="yourhandle" style={{ ...inp, paddingLeft: "28px" }} />
                </div>
              </Field>
            </div>

            <Field label="Bio" hint="Shown on your fan page. Keep it punchy.">
              <textarea value={draft.bio} onChange={e => update("bio", e.target.value.slice(0, 200))}
                placeholder="Exclusive content, direct access, no filters." rows={3} style={{ ...inp, resize: "vertical", minHeight: "74px" }} />
              <div style={{ ...mono, fontSize: "10px", color: "rgba(255,255,255,0.22)", textAlign: "right" }}>{draft.bio.length}/200</div>
            </Field>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              <Field label="Main Specialty" hint="e.g. Streetwear Styling">
                <input type="text" value={draft.specialty} onChange={e => update("specialty", e.target.value)} placeholder="Your niche focus" style={inp} />
              </Field>
              <Field label="Location (optional)">
                <input type="text" value={draft.location} onChange={e => update("location", e.target.value)} placeholder="Dubai, UAE" style={inp} />
              </Field>
            </div>

            <Field label="Website / Link" hint="Additional link displayed below your bio">
              <input type="url" value={draft.websiteUrl} onChange={e => update("websiteUrl", e.target.value)} placeholder="https://yourwebsite.com" style={inp} />
            </Field>

            <div style={{ padding: "18px", background: "rgba(200,169,110,0.04)", border: "1px solid rgba(200,169,110,0.12)", borderRadius: "8px", display: "grid", gap: "14px" }}>
              <div style={{ ...mono, fontSize: "10px", letterSpacing: "0.15em", color: "var(--gold-dim)" }}>PRIMARY CTA BUTTON</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: "14px" }}>
                <Field label="Button Label">
                  <input type="text" value={draft.ctaLabel} onChange={e => update("ctaLabel", e.target.value)} placeholder="Book a Call" style={inp} />
                </Field>
                <Field label="URL">
                  <input type="url" value={draft.ctaUrl} onChange={e => update("ctaUrl", e.target.value)} placeholder="https://yourwebsite.com" style={inp} />
                </Field>
              </div>
            </div>
          </div>
        ...
        <div style={{ position: "sticky", top: "32px" }}>
          <div style={{ ...mono, fontSize: "10px", letterSpacing: "0.18em", color: "rgba(255,255,255,0.22)", marginBottom: "12px" }}>LIVE PREVIEW</div>
          <ProfilePreview draft={draft} avatarPreview={avatarPreview} bannerPreview={bannerPreview} showModal={false} onClose={() => {}} />
          <div style={{ ...mono, fontSize: "10px", color: "rgba(255,255,255,0.2)", marginTop: "10px", textAlign: "center" }}>Updates as you type</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        {msg && (
          <div style={{
            ...
            borderRadius: "6px", color: msgErr ? "#e05555" : "#50d48a",
            ...mono, fontSize: "12px", letterSpacing: "0.06em", whiteSpace: "nowrap",
          }}>{msg}</div>
        )}
        <button onClick={handleSubmit} disabled={saving} style={{
          background: "var(--gold)", border: "none", borderRadius: "4px",
          padding: "14px 36px", ...mono, fontSize: "11px", letterSpacing: "0.18em", fontWeight: 500,
          color: "#0a0800", cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1, minWidth: "200px",
        }}>
      ...
      {showPreview && (
        <ProfilePreview draft={draft} avatarPreview={avatarPreview} bannerPreview={bannerPreview} showModal onClose={() => setShowPreview(false)} />
      )}
    </div>
  );
}
...
export default function ProfileSetupClient({ initialDraft, sources }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Chain into the main onboarding wizard (profile is step 1 of the full flow)
