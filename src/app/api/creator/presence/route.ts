...
// Returns live supporter presence data for the authenticated creator's Command Center.
...
type RawPresenceRow = {
  ...
  updated_at: string;
  supporter_codes_v2: { code: string };
};
...
export async function GET(req: Request) {
  ...
  const [presenceResult, activityResult, spendResult] = await Promise.all([
    ...
    supabase
      ...
      .select(
        "supporter_code_id, last_seen_at, current_page, session_id, updated_at, supporter_codes_v2!inner(code)"
      )
    ...
    supabase
      .from("supporter_activity")
      .select(
        "id, supporter_code_id, activity_type, page, content_id, metadata, created_at, supporter_codes_v2!inner(code)"
      )
      .eq("creator_id", user.id)
      .order("created_at", { ascending: false })
      .limit(50),

    // Aggregate spend per supporter — used for high-value detection
    supabase
      .from("transactions_v2")
      .select("supporter_code_id, creator_earnings, amount")
      .eq("creator_id", user.id)
      .eq("status", "success"),
  ]);
  ...
  for (const tx of (spendResult.data ?? []) as RawTxRow[]) {
    const prev = spendMap.get(tx.supporter_code_id) ?? { total: 0, count: 0 };
    spendMap.set(tx.supporter_code_id, {
  ...
  // ── Shape presence rows ─────────────────────────────────────────────────────
  const allsupporters = (presenceResult.data ?? []).map((row: unknown) => {
    const r = row as RawPresenceRow;
    const spend = spendMap.get(r.supporter_code_id);
    const lastSeen = new Date(r.last_seen_at).getTime();
    return {
      supporter_code_id: r.supporter_code_id,
      code: (r.supporter_codes_v2 as { code: string }).code,
  ...
  const supporters =
    filter === "online"
      ? allsupporters.filter((f) => f.is_online)
      : filter === "recent"
        ? allsupporters.filter((f) => f.is_recent)
        : allsupporters;
  ...
  const onlineNow = allsupporters.filter((f) => f.is_online).length;
  const recentlyActive = allsupporters.filter((f) => f.is_recent).length;
  const vaultViewers = allsupporters.filter(
    (f) => f.is_recent && f.current_page?.toLowerCase().includes("vault")
  ).length;
  const highValueOnline = allsupporters.filter(
