...
// Supporter presence heartbeat — no auth required, code is identity
// POST /api/supporter/presence
// Body: { code: string, page?: string, session_id?: string }
//
// Rate-limited by design: supporters send this every 30s.
// We resolve creator_id via supporter_codes_v2 → content_items_v2 join.
...
export async function POST(req: Request) {
  ...
  // Resolve supporter_code_id and creator_id in one join
  const { data: SupporterCode, error: lookupErr } = await supabase
  ...
  // Upsert presence row — conflict on supporter_code_id
  const { error: upsertErr } = await supabase
    ...
    .upsert(
      ...
      { onConflict: "supporter_code_id" }
    );

  if (upsertErr) {
    console.error("[supporter/presence] upsert error", upsertErr);
    return NextResponse.json({ error: "Presence update failed" }, { status: 500 });
  }

  // Also keep supporter_codes_v2.last_seen_at in sync (backward compat with migration 047)
  await supabase
    .from("supporter_codes_v2")
