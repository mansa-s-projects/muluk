...
export async function PATCH(
  req: NextRequest,
  context: { params: Promise<{ supporter_code: string }> }
) {
  const { supporter_code: supporterCode } = await context.params;
  ...
  if (error) {
    console.error("Supporter CRM update failed", error);
    return NextResponse.json({ error: "Could not update supporter" }, { status: 500 });
  }

  if (!data) {
    return NextResponse.json({ error: "Supporter code not found" }, { status: 404 });
  }
